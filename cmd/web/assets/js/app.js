...
class b extends HTMLElement {
  ...
  _$Ev() {
    this._$Eg = new Promise((t2) => this.enableUpdating = t2), this._$AL = new Map, this._$ES(), this.requestUpdate(), this.constructor.l?.forEach((t2) => t2(this));
  }
  addController(t2) {
    (this._$E_ ??= new Set).add(t2), this.renderRoot !== undefined && this.isConnected && t2.hostConnected?.();
  }
  removeController(t2) {
    this._$E_?.delete(t2);
  }
  ...
  C(t2, s2, i3) {
    this._$AL.has(t2) || this._$AL.set(t2, s2), i3.reflect === true && this._$Em !== t2 && (this._$ET ??= new Set).add(t2);
  }
  async _$EP() {
  ...
  performUpdate() {
    ...
    const s2 = this._$AL;
    try {
      t2 = this.shouldUpdate(s2), t2 ? (this.willUpdate(s2), this._$E_?.forEach((t3) => t3.hostUpdate?.()), this.update(s2)) : this._$Ej();
    } catch (s3) {
  ...
  update(t2) {
    this._$ET &&= this._$ET.forEach((t3) => this._$EO(t3, this[t3])), this._$Ej();
  }
  updated(t2) {
  }
  firstUpdated(t2) {
...
b.elementStyles = [], b.shadowRootOptions = { mode: "open" }, b[d("elementProperties")] = new Map, b[d("finalized")] = new Map, p?.({ ReactiveElement: b }), (a.reactiveElementVersions ??= []).push("2.0.3");
...
// cmd/web/assets/js/schemeSwitcher.ts
class SchemeSwitcher extends s3 {
  constructor() {
    super(...arguments);
    this.cssVarName = "dark";
    this.storageKeyName = this.cssVarName;
  }
  static styles = i`
  :host {
    display: grid;
  ...
  connectedCallback() {
    ...
    if (globalThis.sessionStorage.getItem(this.storageKeyName)) {
      !Boolean(+globalThis.sessionStorage.getItem(this.storageKeyName)) ? document.documentElement.classList.remove(this.cssVarName) : document.documentElement.classList.add(this.cssVarName);
  ...
  switchDarkmode() {
    ...
    !Boolean(+globalThis.sessionStorage.getItem(this.storageKeyName)) ? globalThis.sessionStorage.setItem(this.storageKeyName, "1") : globalThis.sessionStorage.setItem(this.storageKeyName, "0");
    this.mode = Boolean(+globalThis.sessionStorage.getItem(this.storageKeyName));
  }
  render() {
    return x`
      <button @click=${this.switchDarkmode}>${this.mode ? x`🌑` : x`☀️`}</button>
    `;
  }
}
__legacyDecorateClassTS([
  r6()
], SchemeSwitcher.prototype, "mode", undefined);
__legacyDecorateClassTS([
  n4({ type: String, attribute: "css-var-name" })
], SchemeSwitcher.prototype, "cssVarName", undefined);
__legacyDecorateClassTS([
  n4({ type: String, attribute: "storage-key-name" })
], SchemeSwitcher.prototype, "storageKeyName", undefined);
SchemeSwitcher = __legacyDecorateClassTS([
  t3("scheme-switcher")
], SchemeSwitcher);
...
var l3 = (t4) => (r7, ...e6) => {
  ...
  let c4, $2 = 0, f3 = false;
  for (;$2 < i5; ) {
    for (c4 = r7[$2];$2 < i5 && (l4 = e6[$2], s4 = o5(l4)) !== undefined; )
      c4 += s4 + r7[++$2], f3 = true;
    $2 !== i5 && u3.push(l4), n5.push(c4), $2++;
  }
  if ($2 === i5 && n5.push(r7[i5]), f3) {
    const t5 = n5.join("$$lit$$");
    (r7 = a3.get(t5)) === undefined && (n5.raw = n5, a3.set(t5, r7 = n5)), e6 = u3;
  }
  return t4(r7, ...e6);
};
var n5 = l3(x);
var u3 = l3(b2);
// cmd/web/assets/js/TOC.ts
class TOC extends s3 {
  constructor() {
    super(...arguments);
  }
  static styles = [
    ...
    Array(6).fill(0).map((_2, i5) => Object({
      h: i5 + 1,
      fs: `${(8 - i5) / 10}rem`
    })).map((obj) => r(`
      h${obj.h} {
        font-size: ${obj.fs};
        margin-inline-start: ${(obj.h - 1) * 0.5}rem;
      }
    `))
  ];
  headingsMapped;
  ...
  order;
  makeObserver() {
    const getPrev = (n6) => Array(n6 + 1).fill(0);
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((e6) => {
        const idx = this.order[e6.target.id];
        if (!e6.isIntersecting)
          this.style.setProperty(`--heading-${idx}`, "var(--clr)");
        else {
          getPrev(idx).forEach((_2, i5) => this.style.setProperty(`--heading-${i5}`, "var(--accent)"));
        }
      });
    }, {
      rootMargin: "50px",
    ...
    this.headings.forEach((h3) => observer.observe(h3));
  }
  makeHeadings() {
    const makeHash = (s4) => "#" + s4.toLowerCase().replaceAll(/\s[!?]/g, "").replaceAll(" ", "-");
    const article = document.querySelector("article");
    this.headings = article.querySelectorAll("h1, h2, h3, h4, h5, h6");
    this.order = Object.fromEntries([...this.headings].map((h3, i5) => [h3.id, i5]));
    this.headingsMapped = [...this.headings].map((e6, i5) => n5`
      <${i4(e6.tagName)} heading>
        <a class="az" style="color: var(--heading-${i5}, var(--clr))" href=${makeHash(e6.innerHTML)}>${e6.innerHTML}</a>
      </${i4(e6.tagName)}>
    `);
  }
  connectedCallback() {
